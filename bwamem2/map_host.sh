...
then
  ...
  bwa-mem2 -M -t $THREADS $REF $FASTQ_R1 $FASTQ_R2 | samtools view -@ $THREADS_SAM -f 12 -F 256 -uS - -o ${name}_both_unmapped.bam
	samtools sort -@ $THREADS_SAM -n ${name}_both_unmapped.bam -o ${name}_both_unmapped_sorted.bam
	samtools fastq -1 ${name}_clean_1.fastq -2 ${name}_clean_2.fastq -0 /dev/null -s /dev/null -n ${name}_both_unmapped_sorted.bam
  ...
  echo mapping files to host genome""
  bwa-mem2 mem -M -t $THREADS $REF $FASTQ_R1 | samtools view -@ $THREADS_SAM -f 4 -F 256 -uS - -o ${name}_unmapped.bam
  samtools sort -@ $THREADS_SAM -n ${name}_unmapped.bam -o ${name}_unmapped_sorted.bam
  samtools ${name}_unmapped_sorted.bam > ${name}_clean.fastq
